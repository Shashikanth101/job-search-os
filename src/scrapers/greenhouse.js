...
export class GreenhouseScraper extends BaseScraper {
   ...
   */
  async scrape() {
    let response = await fetch(`https://boards-api.greenhouse.io/v1/boards/${this.slug}/jobs?content=true`);
    if (response.status === 404) {
      response = await fetch(`https://job-boards.eu.greenhouse.io/${this.slug}/jobs?content=true`);
      if (!response.ok) {
  ...
  parseJobs(response) {
    ...
    return (response?.jobs ?? [])
      .filter((job) => relevantTerms.some((term) => job.title?.toLowerCase().includes(term)))
      .map((job) => ({
        job_id: job.id,
        title: job.title,
        ...
        job_type: 'Full-time',
        description: (job.content ?? '').replace(/<[^>]*>/g, ' ').trim(),
